...
import * as dbFuncs from '../db/region.ts'
...
describe('getting all regions', () => {
  ...
  it('returns a 500 error when region db call fails', async () => {
    vi.spyOn(dbFuncs, 'getAllRegions').mockRejectedValue(new Error('DB Failed'))

    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    const response = await request(server).get('/api/v1/regions')
